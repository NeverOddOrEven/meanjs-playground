...
(function() {
	// DataManagement Controller Spec
	describe('DataManagementController', function() {
		// Initialize global variables
		var DataManagementController,
			scope,
			$httpBackend,
			$stateParams,
			$location;
		...
		beforeEach(inject(function($controller, $rootScope, _$location_, _$stateParams_, _$httpBackend_) {
			...
			// Initialize the DataManagementController controller.
			DataManagementController = $controller('DataManagementController', {
				$scope: scope
			});
			
			console.info(DataManagementController);
		}));

		it('$scope.getExistingSources() should create an array with at least one article object fetched from XHR', inject(function(Datasets) {
			// Create sample dataset using the Datasets service
			var sampleDataset1 = new Datasets({
				name: 'sample dataset',
				originalFileName: 'sample.xls',
				originalFileLastModified: new Date(1988, 1, 1)
			});
			
			var sampleDataset2 = new Datasets({
				name: 'sample dataset 2',
				originalFileName: 'sample2.xls',
				originalFileLastModified: new Date(2000, 1, 1)
			});

			// Create a sample datasets array that includes the new datasets
			var sampleDatasets = [sampleDataset1, sampleDataset2];

			// Set GET response
			$httpBackend.expectGET('datamanagement/listexistingsources')
				.respond(sampleDatasets);
				
			// Run controller functionality
			console.info(scope);
			scope.getExistingSources();
			$httpBackend.flush();
